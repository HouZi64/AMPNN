...
def add_padding_raw(features, max_padding):
    padded = []
    ...
    for i in range(len(features)):
        ...
        if (size < max_padding):
            ...
            xLeft = xDiff // 2
            xRight = xDiff - xLeft
            px = np.pad(px, (xLeft, xRight), mode='constant')

        padded.append(px)

    return padded

def add_padding_rms(features, max_padding):
    padded = []
...
def plot_train_history(history, x_ticks_vertical=False):
    ...
    max_accuracy = max(history['acc'])
    max_val_accuracy = max(history['val_acc'])
    # max_val_accuracy = max(history['val_acc'])
    # x pos for loss / acc min/max
    min_loss_x = history['loss'].index(min_loss)
    min_val_loss_x = history['val_loss'].index(min_val_loss)
    max_accuracy_x = history['acc'].index(max_accuracy)
    max_val_accuracy_x = history['val_acc'].index(max_val_accuracy)
    ...
    plt.plot(history['acc'], alpha=0.7)
    ...
    if (x_ticks_vertical):
        plt.xticks(np.arange(0, len(history['acc']), 5.0), rotation='vertical')
    else:
        plt.xticks(np.arange(0, len(history['acc']), 5.0))
...
def plot_train_history(history, x_ticks_vertical=False):
    ...
    plt.plot(history['loss'], color="#1f77b4", alpha=0.7)
    plt.plot(history['val_loss'], color="#ff7f0e", linestyle="--")
    plt.plot(min_loss_x, min_loss, marker='s', markersize=15, color="#1f77b4", alpha=0.7, label='Inline label')
    plt.plot(min_val_loss_x, min_val_loss, marker='o', markersize=15, color="#ff7f0e", alpha=7, label='Inline label')
    #plt.title('Model loss', fontsize=25)
    plt.ylabel('Loss', fontsize=5)
    plt.xlabel('Epoch', fontsize=5)
    plt.xticks(fontsize=5,color='black',fontproperties = 'Times New Roman',size=30)
    plt.yticks(fontsize=5,color='black',fontproperties = 'Times New Roman',size=30)
    plt.legend(['Train',
                'Test',
                ('Min_loss: %.3f' % min_loss),
                ('Min_val_loss: %.3f' % min_val_loss)],
                loc='upper right',
                fancybox=True,
                framealpha=0.9,
                shadow=True,
    ...
    if (x_ticks_vertical):
        plt.xticks(np.arange(0, len(history['loss']), 200.0), rotation='vertical')
    else:
        plt.xticks(np.arange(0, len(history['loss']), 200.0))
    ...
    plt.plot(history['acc'], alpha=0.7)
    plt.plot(history['val_acc'], linestyle="--")
    plt.plot(max_accuracy_x, max_accuracy, marker='s', markersize=15, color="#1f77b4", alpha=7)
    plt.plot(max_val_accuracy_x, max_val_accuracy, marker='o', markersize=15, color="#ff7f0e", alpha=7)
    ...
    plt.legend(['Train',
                'Test',
                ('Max_accuracy: %.3f' % max_accuracy),
                ('Max_val_accuracy: %.3f' % max_val_accuracy)],
                loc='lower right',
                fancybox=True,
                framealpha=0.9,
                shadow=True,
    ...
    if (x_ticks_vertical):
        plt.xticks(np.arange(0, len(history['acc']), 200.0), rotation='vertical',size=30)
    else:
        plt.xticks(np.arange(0, len(history['acc']), 200.0),size=30)
...
def plot_confusion_matrix(cm,
    ...
    im = ax.imshow(cm, interpolation='nearest', cmap=cmap)
    # color bar
    cb = ax.figure.colorbar(im, ax=ax)#图例
    ...
    #ax.set_title(label=title, fontdict=font1)

    plt.xticks(fontsize=5,color='black',fontproperties = 'Times New Roman',size=30)
    plt.yticks(fontsize=5,color='black',fontproperties = 'Times New Roman',size=30)# 刻度大小
    plt.tick_params()
